...
stores them as .jpeg files. and then extracts faces from the said images, builds
...
def generate(path_root = 'D:\deepfake_database\dfdc_train_part_1', save_path = "train_files", capture_sec = 5):
    ...
    for i in range(len(list_name)):
        ...
        root_folder = save_path + "/"+ root_name
        ...
        def file_name_generator(j):
            filename = ''
            if(j<10):
                file_name = "00"+str(j)+ ".jpeg"
            elif(j<100 and j>=10):
                file_name = "0" + str(j) + ".jpeg"
            else:
